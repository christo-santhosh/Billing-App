...
def generate_invoice_pdf(invoice_id):
    ...
    data_header = [
        [
            ...
            Paragraph(
                f"<b>Date:</b> {invoice.date.strftime('%B %d, %Y - %I:%M %p')}", right_align_style)
        ],
        [
            Paragraph(
                f"<b>Ward:</b> {ward.ward_name} ({ward.ward_number})", normal_style),
            Paragraph(
                f"<b>Payment Method:</b> {invoice.payment_method}", right_align_style)
        ],
        [
    ...
    item_table.setStyle(TableStyle([
        ...
        ('LINEABOVE', (-2, -1), (-1, -1), 2,
         colors.HexColor('#16a34a')),  # Line above total
        ('LINEBELOW', (-2, -1), (-1, -1), 2,
         colors.HexColor('#16a34a')),  # Line below total
    ]))
